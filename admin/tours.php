...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['flash_message'] = ['status' => 'danger', 'message' => 'Không thể cập nhật trạng thái.'];
    }
    $stmt->close();
    header('Location: tours.php');
    $conn->close();
                                                ...
                                                while ($row = $result->fetch_assoc()) {
                                                    ...
                                                    echo "<td>
                                                        <form action='tours.php' method='POST' class='toggle-form'>
                                                            ...
                                                            <label class='switch'>
                                                                <input type='checkbox' " . ($row['status'] === 'active' ? 'checked' : '') . " onchange='this.closest(\".toggle-form\").submit()'>
