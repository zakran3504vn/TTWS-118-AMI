...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE news SET title = ?, summary = ?, content = ?, category = ?, image = ?, isTop = ?, slug = ? WHERE id = ? AND category != 'Visa'");
    $stmt->bind_param('sssssssi', $title, $summary, $content, $category, $image, $isTop, $slug, $id);
    if ($stmt->execute()) {
...
<head>
    ...
    <title>Chỉnh Sửa Bài Viết Tin Tức</title>
                ...
                <div class="container-fluid p-0">
                    <div class="row justify-content-center">
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="dashboard_header_title">
                                            <h3>Chỉnh Sửa Bài Viết Tin Tức</h3>
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="dashboard_breadcam text-end">
                                            <p><a href="../index.php">Dashboard</a> <i class="fas fa-caret-right"></i> <a href="other_news.php">Tin Tức</a> <i class="fas fa-caret-right"></i> Chỉnh Sửa Bài Viết</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-lg-8">
                            <div class="card">
                                <div class="card-body">
                                    <form action="edit_news.php?id=<?php echo urlencode($id); ?>" method="POST">
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" id="category" name="category" required>
                                                <?php foreach ($categories as $cat): ?>
                                                    <option value="<?php echo htmlspecialchars($cat); ?>" <?php echo $news['category'] === $cat ? 'selected' : ''; ?>><?php echo htmlspecialchars($cat); ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="mb-3">
